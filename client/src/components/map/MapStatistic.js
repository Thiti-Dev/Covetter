		...
		return (
			<View style={styles.cardStatistics}>
				...
				>
					...
					{this.state.statisticsData === null ? '' : ` +${this.state.statisticsData.new.confirmed}`}
				</Text>
            ...
            <Text style={styles.border}> die </Text>
					...
					<View style={styles.dataDetailsContainer}>
						<View style={[ styles.dataDetails, { backgroundColor: '#DFCE30' } ]}>
							<Text>Healing</Text>
							<Text style={{ fontWeight: 'bold', fontSize: 28, color: '#3D3D3D' }}>
								{this.state.statisticsData === null ? (
									'❤️'
								) : (
					...
					<View style={styles.dataDetailsContainer}>
						<View style={[ styles.dataDetails, { backgroundColor: '#74FF68' } ]}>
							<Text>Healed</Text>
							<Text style={{ fontWeight: 'bold', fontSize: 28, color: '#3D3D3D' }}>
								{this.state.statisticsData === null ? '❤️' : this.state.statisticsData.total.recovered}
							</Text>
							<View style={styles.dataDetailsSecond}>
								<Text>
									{this.state.statisticsData === null ? (
					...
					<View style={styles.dataDetailsContainer}>
						<View style={[ styles.dataDetails, { backgroundColor: '#FF6161' } ]}>
							<Text>Die</Text>
							<Text style={{ fontWeight: 'bold', fontSize: 28, color: '#3D3D3D' }}>
								{this.state.statisticsData === null ? '❤️' : this.state.statisticsData.total.death}
							</Text>
							<View style={styles.dataDetailsSecond}>
								<Text>
									{this.state.statisticsData === null ? (
