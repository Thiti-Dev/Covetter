...
export default class ModalPop extends Component {
	...
	async onCommitCharity() {
		...
		if (lat != 0 && lng != 0) {
			...
			const _body = createFormData(staged_image_response, { lat, lng, description, estimated_hour: hour });
			try {
			...
			<Modal animationType="slide" transparent={true} visible={this.props.modalVisible}>
				...
				>
					...
					>
						...
						>
							<Image
								source={staged_image || require('../../assets/images/add-item.png')}
								style={{
									height: '100%',
									width: '100%'
								}}
						...
						<Text style={{ margin: 10, fontSize: 14, fontFamily: 'Promt-Regular' }}>ชื่อกิจกรรม</Text>
						<TextInput
							value={description}
